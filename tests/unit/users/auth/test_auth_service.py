...
def test_get_yandex_redirect_url__success(auth_service: AuthService, settings: Settings):
    yandex_redirect_url = auth_service.get_yandex_redirect_url()
    assert yandex_redirect_url == settings.yandex_redirect_url



async def test_regenerate_access_token__success(auth_service: AuthService, settings: Settings):
    user_id = 1

    access_token = auth_service.generate_token(user_id)
...
async def test_yandex_auth__success(auth_service: AuthService):
    ...
    user = await auth_service.yandex_auth(code)
    decode_user_id = auth_service.get_user_id_from_access_token(user.access_token)
